...
router.put('/:id',async function(req,res,next) {
    ...
    if(club == undefined || Object.keys(body).length === 0 ) {
        ...
        db.query("DELETE FROM igraci WHERE naziv_kluba = '" + id +"'")
        db.query("DELETE FROM pomocni_treneri WHERE naziv_kluba = '" + id +"'")

   
        db.query("DELETE FROM osnovni_podaci WHERE naziv_kluba = '" + id +"'")
        ...
        if(await (await db.query("SELECT FROM dvorane where ime_dvorane = '"+ ime_dvorane +"'")).rowCount == 0)
            db.query("insert into dvorane (ime_dvorane,kapacitet_dvorane) values ('" + ime_dvorane + "','" + kapacitet_dvorane + "')");

        
        db.query("insert into osnovni_podaci (naziv_kluba,mjesto_sjedista,glavni_trener,godina_osnutka,pozicija2021,ime_dvorane,naziv_lige) values('"+naziv_kluba+"','"+mjesto_sjedista+"','"+glavni_trener+"','"+godina_osnutka+"','"+pozicija2021+"','"+ime_dvorane+"','"+naziv_lige+"')");

        db.query("insert into pomocni_treneri (punoime_pomocnika,naziv_kluba) values('"+punoime_pomocnika+"','"+naziv_kluba+"')")
